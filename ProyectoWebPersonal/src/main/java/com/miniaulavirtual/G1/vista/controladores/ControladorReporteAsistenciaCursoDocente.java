...
@ManagedBean(name = "asistencia1")
@ViewScoped
public class ControladorReporteAsistenciaCursoDocente implements Serializable{
	@EJB
	...
	public void cargarAsistencias() {
		List<Asistencia> temp = new ArrayList<>();
		...
		for(int i = 0; i < clases.size(); i ++) {
			...
			for(int j = 0; j < temp.size(); j ++) {
				if(temp.get(j).getAlumno().getIdAlumno() == alumnoSeleccionado.getIdAlumno()) {
